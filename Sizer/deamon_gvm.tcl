...
proc .compile_circuit {} {
    ...
    foreach name [array names ::Ids_equations] {
	    ...
	    p2c $name:gm $::gm_equations($name)
            $name:go=>$::go_equations($name)
	    p2c $name:go $::go_equations($name)
        } else {
            $name:g=>$::g_equations($name)
	    p2c $name:g $::g_equations($name)
        }
    }
    foreach name [array names ::G_equations] {
        Gds_$name=>$::G_equations($name)
        p2c Gds_$name:g $::G_equations($name)
    ...
    *c "@Ted=1/([DET ::KCL]);"
	...
	set derived_poly [present_poly dg_d$c Det_d$c]
        Det:$c=>$derived_poly
        Ted:$c=>-Det:$c/(Det*Det)
	p2c @Det:$c $derived_poly
