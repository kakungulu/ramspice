...
#    return "HTTP/1.0 $code ???\nContent-Type: text/html; charset=ISO-8859-1\nConnection: close\nContent-length: [expr int(8*ceil([string length $body]/8.0))]\n$head\n$body"
    return "HTTP/1.0 $code ???\nContent-Type: text/html; charset=ISO-8859-1\nConnection: close\n$head\n$body"
}
set ::solution {}
proc Log: {args} {
...
proc GET {args} {
    ...
    return [respond 200 "<html><body>[subst $::HTML]<font color=\"red\">$error</font>$::solution</body></html>"]
}
proc post_calculate {} {}
...
proc gen_form {} {
	...
	if {[info exists ::simulated($field)]} {
	    ...
	    Info: value=$value sim=$::simulated($field)
	    set abs_err [eng [expr $value-$::simulated($field)] $::const($field,unit)]
	    set rel_err [eng [expr ($value/$::simulated($field)-1)*100] %]
	}
	...
	if {[lsearch $::open_fields $field]==-1} {
	    if {[lsearch $::randomized $field]==-1} {
                append retval "<tr><td>$field_token</td><td><input $::const($field,type) name=\"$field\" value=\"$value\" width=\"$width\"></td><td>$::const($field,unit)</td><td></td><td>$sim_value</td><td>$abs_err</td><td>$rel_err</td></tr>"
	    } else {
                append retval "<tr><td>$field_token</td><td><input $::const($field,type) name=\"$field\" value=\"#\" width=\"$width\"></td><td>[eng $value $::const($field,unit)]</td><td>Random</td><td>$sim_value</td><td>$abs_err</td><td>$rel_err</td></tr>"
	    }	
	} else {
	    if {![info exists ::step_lookup($field)]} {
            ...
            append retval "<tr><td>$field_token</td><td><input $::const($field,type) name=\"$field\" value=\"\" width=\"$width\"></td><td>[eng $value $::const($field,unit)]</td><td>$step_reference</td><td>$sim_value</td><td>$abs_err</td><td>$rel_err</td></tr>"
	}
    }
    append retval "</table>"
    return $retval
...
proc LUT {name args} {
    if {![@ /look_up_tables/$::device/$name/$::process exists]} {
        Warning: look up in $name:  $args => does not exist
	return 0
    }
    set retval [uplevel \#0 "@ /look_up_tables/$::device/$name/$::process calc $args"]
...
proc calculate {{varlist {}}} {
   ...
   set error {}
   for {set iteration 1} {$iteration<=64} {incr iteration} {
       set ::max_change 0
...
source $::env(RAMSPICE)/tests/single_transistor_function.tcl
