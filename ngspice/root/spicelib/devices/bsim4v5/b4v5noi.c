...
double freq, double temp)
{
    ...
    * log(MAX(((N0 + here->BSIM4v5nstar) / (Nl + here->BSIM4v5nstar)), N_MINLOG));
    ...
    //#Info: "BSIM4v5Eval1ovFNoise %x %x Vds=%g L=%g Leff=%g freq=%g Weff=%g Vdseff=%g temp=%g =>>  flicker=%g" model here Vds here->BSIM4v5l Leff freq pParam->BSIM4v5weff here->BSIM4v5Vdseff temp Ssi*EffFreq
    return Ssi;
}
...
double *OnDens)
{
    ...
    //#Info: "BSIM4v5noise %x %x" inModel ckt
    ...
    {    for (here = model->BSIM4v5instances; here != NULL;
        ...
        {    pParam = here->pParam;
    //#Info: "BSIM4v5noise %x %x here=%x" inModel ckt here
            switch (operation)
            {  case N_OPEN:
