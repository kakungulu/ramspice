    ...
    {    for (here = model->BSIM4v5instances; here != NULL; 
	    ...
	    if (DEBUG_MODEL>0) {
            ...
            */
            ...
            line1000:  ;
	    InnerCalc=get_bsim4v5_inner_calc(here);
...
BSIM4v5InnerCalc *get_bsim4v5_inner_calc(BSIM4v5instance *i_inst) {
    BSIM4v5InnerCalc *InnerCalc;
    if (BSIM4v5InnerCalcList) {
        for (InnerCalc=BSIM4v5InnerCalcList;InnerCalc;InnerCalc=InnerCalc->next) if (InnerCalc->inst==i_inst) return(InnerCalc);
        for (InnerCalc=BSIM4v5InnerCalcList;InnerCalc->next;InnerCalc=InnerCalc->next) {}
	InnerCalc->next=(BSIM4v5InnerCalc *)malloc(sizeof(BSIM4v5InnerCalc));
	InnerCalc=InnerCalc->next;
	InnerCalc->inst=i_inst;
	InnerCalc->next=NULL;
    } else {
        BSIM4v5InnerCalcList=(BSIM4v5InnerCalc *)malloc(sizeof(BSIM4v5InnerCalc));
	BSIM4v5InnerCalcList->inst=i_inst;
	BSIM4v5InnerCalcList->next=NULL;
	InnerCalc=BSIM4v5InnerCalcList;
    }
    return(InnerCalc);
}
