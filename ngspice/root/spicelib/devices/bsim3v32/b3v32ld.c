    ...
    for (; model != NULL; model = model->BSIM3v32nextModel) {    
        for (here = model->BSIM3v32instances; here != NULL; here = here->BSIM3v32nextInstance) {
            ...
            */
            ...
            line1000:  ;
	    InnerCalc=get_bsim3v32_inner_calc(here);
            #Foreach: var $::bsim3v32_internal_calculation_vars {
                InnerCalc->$var=$var;
            }
...
BSIM3v32InnerCalc *get_bsim3v32_inner_calc(BSIM3v32instance *i_inst) {
    BSIM3v32InnerCalc *InnerCalc;
    if (BSIM3v32InnerCalcList) {
        for (InnerCalc=BSIM3v32InnerCalcList;InnerCalc;InnerCalc=InnerCalc->next) if (InnerCalc->inst==i_inst) return(InnerCalc);
        for (InnerCalc=BSIM3v32InnerCalcList;InnerCalc->next;InnerCalc=InnerCalc->next) {}
	InnerCalc->next=(BSIM3v32InnerCalc *)malloc(sizeof(BSIM3v32InnerCalc));
	InnerCalc=InnerCalc->next;
	InnerCalc->inst=i_inst;
	InnerCalc->next=NULL;
    } else {
        BSIM3v32InnerCalcList=(BSIM3v32InnerCalc *)malloc(sizeof(BSIM3v32InnerCalc));
	BSIM3v32InnerCalcList->inst=i_inst;
	BSIM3v32InnerCalcList->next=NULL;
	InnerCalc=BSIM3v32InnerCalcList;
    }
    return(InnerCalc);
}
