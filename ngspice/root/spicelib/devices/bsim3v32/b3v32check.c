...
BSIM3v32checkModel (BSIM3v32model *model,BSIM3v32instance *here,CKTcircuit *ckt)
{
    ...
    int Fatal_Flag = 0;
    FILE *fplog;
//    #Info: "BSIM3v32checkModel"
    NG_IGNORE(ckt);
    
    if ((fplog = fopen("b3v32check.log","w")) != NULL)
    {   pParam = here->pParam;
        
        fprintf (fplog,"BSIM3 Model (Supports: v3.2,v3.2.2,v3.2.3,v3.2.4)\n");
        fprintf (fplog,"Parameter Checking.\n");
        fprintf (fplog,"Model = %s\n",model->BSIM3v32modName);
        fprintf (fplog,"W = %g,L = %g,M = %g\n",here->BSIM3v32w,here->BSIM3v32l,here->BSIM3v32m);
        ...
        if (model->BSIM3v32acmMod == 0) {
            ...
            model->BSIM3v32unitLengthGateSidewallJctCap > 0.0)
            {
                ...
                {   fprintf(fplog,"Warning: Pd = %g is less than W=%g.\n",here->BSIM3v32drainPerimeter,pParam->BSIM3v32weff);
                    //printf("Warning: Pd = %g is less than W=%g.\n",here->BSIM3v32drainPerimeter,pParam->BSIM3v32weff);
                }
                if (here->BSIM3v32sourcePerimeter < pParam->BSIM3v32weff)
                {   fprintf(fplog,"Warning: Ps = %g is less than W=%g.\n",here->BSIM3v32sourcePerimeter,pParam->BSIM3v32weff);
                    //printf("Warning: Ps = %g is less than W=%g.\n",here->BSIM3v32sourcePerimeter,pParam->BSIM3v32weff);
                }
            }
        }
