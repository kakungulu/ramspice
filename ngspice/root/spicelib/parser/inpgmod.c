        ...
        while (*line != 0) {
            ...
            for (j = 0; j < *(ft_sim->devices[modtmp->INPmodType]->numModelParms); j++) {
                ...
                if (strcmp(parm, ft_sim->devices[modtmp->INPmodType]->modelParms[j].keyword) == 0) {
                    ...
                    val, NULL);
                    // Info: "Setting param %s '%s' to %g" parm ft_sim->devices[modtmp->INPmodType]->modelParms[j].description val->rValue 
                    if (error)
                    return error;
                    break;
                }
            ...
            if (strcmp(parm, "level") == 0) {
                ...
                if (endptr == parm) { /* it was no number - it is really a string */
                    temp = TMALLOC(char, 256 + strlen(parm));
                    (void) sprintf(temp,
                    "unrecognized parameter (%s) - ignored (ystatter)", parm);
