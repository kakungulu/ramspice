...
void
cm_core(Mif_Private_t *mif_private)
{
    ...
    mode = mif_private->param[6]->element[0].ivalue;
    ...
    if (HYSTERESIS != mode) {   /******** pwl mode *****************/
        ...
        input_domain = mif_private->param[4]->element[0].rvalue;
        area = mif_private->param[2]->element[0].rvalue;
        length = mif_private->param[3]->element[0].rvalue;

        size = mif_private->param[0]->size;

        H = (Mif_Value_t*) &mif_private->param[0]->element[0].rvalue;
        B = (Mif_Value_t*) &mif_private->param[1]->element[0].rvalue;

        /* See if input_domain is absolute...if so, test against   */
        /* breakpoint segments for violation of 50% rule...        */
        if (mif_private->param[5]->element[0].bvalue == MIF_FALSE)
            for (i = 0; i < size - 1; i++)
        ...
        mmf_input = mif_private->conn[0]->port[0]->input.rvalue;
        ...
        if (H_input <= (H[1].rvalue + H[0].rvalue) / 2.0) {/*** H_input below lowest midpoint ***/
            ...
            for (i = 1; i < size; i++)
                if (H_input < (H[i].rvalue + H[i+1].rvalue) / 2.0) {
                    ...
                    if (mif_private->param[5]->element[0].bvalue == MIF_TRUE) {  /* Translate input_domain */
                        /* into an absolute....   */
        ...
        if (mif_private->circuit.anal_type != MIF_AC) {        /* Output DC & Transient Values */
            mif_private->conn[0]->port[0]->output.rvalue = flux_out;
             mif_private->conn[0]->port[0]->partial[0].port[0] = dout_din;
        } else {                    /* Output AC Gain */
        ...
        in_low = mif_private->param[7]->element[0].rvalue;
        in_high = mif_private->param[8]->element[0].rvalue;
        hyst = mif_private->param[9]->element[0].rvalue;
        out_lower_limit = mif_private->param[10]->element[0].rvalue;
        out_upper_limit = mif_private->param[11]->element[0].rvalue;
        input_domain = mif_private->param[4]->element[0].rvalue;
        ...
        if (mif_private->param[5]->element[0].bvalue == MIF_TRUE)
        ...
        *hyst_state = *old_hyst_state;
        ...
        if (mif_private->circuit.anal_type != MIF_AC) {     /* DC & Transient Analyses */
            mif_private->conn[0]->port[0]->output.rvalue = out;
             mif_private->conn[0]->port[0]->partial[0].port[0] = pout_pin;
        } else {                      /* AC Analysis */
