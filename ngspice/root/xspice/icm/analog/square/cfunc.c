...
#line 1 "./analog/square/cfunc.mod"
...
void
cm_square(Mif_Private_t *mif_private)
{
    ...
    t_rise = mif_private->param[5]->element[0].rvalue;
    t_fall = mif_private->param[6]->element[0].rvalue;
    ...
    if (mif_private->circuit.anal_type == MIF_DC) {
        ...
        *phase = *phase1 + freq*(mif_private->circuit.time - mif_private->circuit.t[1]);
        ...
        if ((time1 <= mif_private->circuit.time) && (mif_private->circuit.time <= time2)) {
            ...
            time3 = mif_private->circuit.t[1] + (1 - dphase) / freq;
            time4 = time3 + t_fall;
            ...
            mif_private->conn[1]->port[0]->output.rvalue = output_low + ((mif_private->circuit.time - time1) / (time2 - time1)) *
                (output_hi - output_low);

        } else if ((time2 <= mif_private->circuit.time) && (mif_private->circuit.time <= time3)) {

            time3 = mif_private->circuit.t[1] + (1.0 - dphase) / freq;
            time4 = time3 + t_fall;

            if (mif_private->circuit.time < time3)
                cm_analog_set_temp_bkpt(time3);

            cm_analog_set_temp_bkpt(time4);

            mif_private->conn[1]->port[0]->output.rvalue = output_hi;
            ...
            time1 = mif_private->circuit.t[1] + ((1-d_cycle) - dphase) / freq;
            time2 = time1 + t_rise;

            if (mif_private->circuit.time < time4)
                cm_analog_set_temp_bkpt(time4);

            cm_analog_set_temp_bkpt(time1);
            cm_analog_set_temp_bkpt(time2);

            mif_private->conn[1]->port[0]->output.rvalue = output_hi + ((mif_private->circuit.time - time3) / (time4 - time3)) *
