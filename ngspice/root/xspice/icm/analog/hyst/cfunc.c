...
void cm_hyst(Mif_Private_t *mif_private)  /* structure holding parms, 
                                       inputs, outputs, etc.     */
{
    ...
    hyst = mif_private->param[2]->element[0].rvalue;
    out_lower_limit = mif_private->param[3]->element[0].rvalue;
    out_upper_limit = mif_private->param[4]->element[0].rvalue;                         
    input_domain = mif_private->param[5]->element[0].rvalue;
    ...
    if (mif_private->param[6]->element[0].bvalue == MIF_TRUE)        /* Set range to absolute value */
