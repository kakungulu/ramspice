...
#line 1 "./digital/dac_bridge/cfunc.mod"
...
void cm_dac_bridge(Mif_Private_t *mif_private) 

{
    ...
    out_low = mif_private->param[0]->element[0].rvalue;
    out_high = mif_private->param[1]->element[0].rvalue;
    t_rise = mif_private->param[4]->element[0].rvalue;
    t_fall = mif_private->param[5]->element[0].rvalue;
    ...
    if (!mif_private->param[0]->is_null && !mif_private->param[1]->is_null && 
         mif_private->param[2]->is_null ) {
       out_undef = out_low + (out_high - out_low) / 2.0;
    }
    else {
       out_undef = mif_private->param[2]->element[0].rvalue;
    }                                 
    ...
    if (mif_private->circuit.init) {  /*** Test for INIT == TRUE. If so, allocate storage, etc. ***/
        ...
        for (i=0; i<size; i++) {
            in[i] = ((Digital_t*)(mif_private->conn[0]->port[i]->input.pvalue))->state;
        }
        ...
        for (i=0; i<size; i++) { /* assign addresses */
            ...
            switch (in[i]) {
                ...
                case ZERO: out[i] = out_old[i] = out_low;
                        mif_private->conn[1]->port[i]->output.rvalue = out_old[i];
                        break;
    ...
    case EVENT:  /** discrete call... **/
        ...
        for (i=0; i<size; i++) {
            ...
            if (in[i] != in_old[i]) { /* if there has been a change... */
                ...
                cm_analog_set_perm_bkpt(mif_private->circuit.time);
    ...
    case ANALOG:    /** analog call... **/
        ...
        time_inc = mif_private->circuit.time - mif_private->circuit.t[1];
        ...
        for (i=0; i<size; i++) {  
            ...
            if ( in_old[i] == in[i] ) {    /* There has been no change in 
                ...
                case ONE:        /* rising for all outputs */
                        ...
                        test = mif_private->circuit.time + (fraction * t_rise);
                ...
                case UNKNOWN:    /* may be rising or falling */
                        ...
                        if ( out_undef > out[i] ) { /* rising to U */
                            ...
                            test = mif_private->circuit.time + (fraction * t_fall);
                            cm_analog_set_perm_bkpt(test);
                        }
                        break;
        ...
        for (i=0; i<size; i++) {
            ...
            mif_private->conn[1]->port[i]->output.rvalue = out[i];
