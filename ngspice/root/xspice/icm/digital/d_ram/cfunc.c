...
#line 1 "./digital/d_ram/cfunc.mod"
...
void cm_d_ram(Mif_Private_t *mif_private) 

{
    ...
    address_size = mif_private->conn[2]->size;    
    ...
    select_size = mif_private->conn[4]->size;
    select_value = mif_private->param[0]->element[0].ivalue;
    word_width = mif_private->conn[0]->size;
    ...
    if(mif_private->circuit.init) {  /* initial pass */ 
        ...
        for (i=0; i<word_width; i++) {
            mif_private->conn[0]->port[i]->load = mif_private->param[3]->element[0].rvalue;
        }

        for (i=0; i<address_size; i++) {
            mif_private->conn[2]->port[i]->load = mif_private->param[4]->element[0].rvalue;
        }                        

        mif_private->conn[3]->port[0]->load = mif_private->param[6]->element[0].rvalue;

        for (i=0; i<select_size; i++) {
            mif_private->conn[4]->port[i]->load = mif_private->param[5]->element[0].rvalue;
        }
    ...
    /**** retrieve inputs ****/
    *write_en = ((Digital_t*)(mif_private->conn[3]->port[0]->input.pvalue))->state;

    address_changed = FALSE;
    address_unknown = FALSE;
    for (i=0; i<address_size; i++) {
        address[i] = ((Digital_t*)(mif_private->conn[2]->port[i]->input.pvalue))->state;
    ...
    *select = ONE;
    for (i=0; i<select_size; i++) { /* compare each bit in succession
                                       ...
                                       select input lines.          */
        if ( (d_test = ((Digital_t*)(mif_private->conn[4]->port[i]->input.pvalue))->state) != (int_test = (select_value & 0x0001)) ) {
            *select = ZERO;
        }
    ...
    if (0.0 == mif_private->circuit.time) {   /****** DC analysis...output w/o delays ******/
        ...
        out = (Digital_State_t) mif_private->param[1]->element[0].ivalue;
        for (i=0; i<word_width; i++) {
        ...
        if ( (ONE==*select) && (ZERO==*write_en) ) {
            ...
            for (i=0; i<word_width; i++) {
                ((Digital_t*)(mif_private->conn[1]->port[i]->output.pvalue))->state = (Digital_State_t) mif_private->param[1]->element[0].ivalue;
            ...
            for (i=0; i<word_width; i++) {
                ...
                ((Digital_t*)(mif_private->conn[1]->port[i]->output.pvalue))->strength = HI_IMPEDANCE;
            }
        }
    }
            ...
            case ONE: /** chip is selected **/
                if ( ZERO == *write_en ) {   /* need to retrieve new output */
                    for (i=0; i<word_width; i++) { 
                        ...
                        ((Digital_t*)(mif_private->conn[1]->port[i]->output.pvalue))->state = out;                              
                        ((Digital_t*)(mif_private->conn[1]->port[i]->output.pvalue))->strength = STRONG;                              
                    ...
                    for (i=0; i<word_width; i++) {
                        if (address_unknown) {
                            ...
                            out = ((Digital_t*)(mif_private->conn[0]->port[i]->input.pvalue))->state;
