...
CountTwins( MatrixPtr Matrix, int Col, ElementPtr *ppTwin1, ElementPtr *ppTwin2 )
{
	...
	if (ABS(pTwin1->Real) == 1.0)
	    ...
	    Row = pTwin1->Row;
            ...
            if ((pTwin2 != NULL) && (ABS(pTwin2->Real) == 1.0))
            {
...
spErrorMessage(MatrixPtr Matrix, FILE *Stream, char *Originator)
{
    ...
    /* Print particular error message.  Do not use switch statement
     * because error codes may not be unique.  */
    if (Error == spPANIC)
