...
12. Get next token.  Depending upon token type (double or array) do
    the following:
    ...
    */
    for(i = 0; i < DEVices[type]->DEVpublic.num_conn; i++) {
        ...
        */
        ...
        /* =====  process connection as appropriate for double or array  ====== */
        if(! conn_info->is_array) {     /* a double connection - the simpler case */
            ...
            MIFget_port(ckt,
                        ...
                        conn_info,
                        i,                  /* connection index */
                        0,                  /* port index for double connection */
                        &status);
