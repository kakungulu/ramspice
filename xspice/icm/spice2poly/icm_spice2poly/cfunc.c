...
#line 1 "./spice2poly/icm_spice2poly/cfunc.mod"
...
void spice2poly (Mif_Private_t *mif_private)
{
    ...
    if(mif_private->circuit.anal_type == MIF_AC) {
        for(i = 0; i < num_inputs; i++) {
            mif_private->conn[1]->port[0]->ac_gain[0].port[i].real = mif_private->inst_var[0]->element[i].rvalue;
            mif_private->conn[1]->port[0]->ac_gain[0].port[i].imag = 0.0;
        }
        return;
    }
    ...
    num_coefs = mif_private->param[0]->size;

    coef = (double *) malloc((size_t) num_coefs * sizeof(double));
    for(i = 0; i < num_coefs; i++)
        coef[i] = mif_private->param[0]->element[i].rvalue;
    ...
    mif_private->conn[1]->port[0]->output.rvalue = sum;
