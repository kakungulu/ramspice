...
#line 1 "./analog/s_xfer/cfunc.mod"
...
void cm_s_xfer(Mif_Private_t *mif_private)  /* structure holding parms, inputs, outputs, etc.     */
{
    ...
    num_size = mif_private->param[2]->size;                    
    den_size = mif_private->param[3]->size;                    
    if ( mif_private->param[5]->is_null ) {
        denormalized_freq = 1.0;          
    }
    else {
        denormalized_freq = mif_private->param[5]->element[0].rvalue;                    
    }
    ...
    if (mif_private->circuit.init==1) {  /* First pass...allocate storage for previous values... */
...
/*  We have to allocate memory and use cm_analog_alloc, because the ITP variables
   ...
   /*     ITP_VAR_SIZE(den) = den_size;  */
    ...
    if (mif_private->circuit.time == 0.0) {  /* First pass...set initial conditions... */
        ...
        for (i=0; i<den_size-1; i++) {   /* Note...do NOT set the highest   */
            ...
            if ( mif_private->param[4]->is_null ) {
                ...
                *(integrator[i]) = *(old_integrator[i]) = 
                                       mif_private->param[4]->element[den_size-2-i].rvalue;
            }
        }
        ...
        for (i=0; i<num_size; i++) {
            *(num_coefficient[i]) =     mif_private->param[2]->element[num_size-1-i].rvalue;
        ...
        for (i=0; i<den_size; i++) {
            *(den_coefficient[i]) =     mif_private->param[3]->element[den_size-1-i].rvalue;
            if ( denormalized_freq != 1.0 ) {
		...
		if ( (factor = *den_coefficient[den_size-1]) != 1.0 ) {
            ...
            *gain = mif_private->param[1]->element[0].rvalue / factor; 
        }
    ...
    /**** AC Analysis ************************************/
        ...
        for (i=0; i<num_size; i++) {
            ...
            if (modf(divide_integer/2.0,&temp) > 0.0 ) { /* Negative coefficient       */
                ...
                if (frac > 0.0 ) {  /** Odd Powers of "s" **/
                    acc_num.imag = acc_num.imag - *(num_coefficient[i]) * pow(mif_private->circuit.frequency,i) * (*gain); 
                }
                else {                      /** Even Powers of "s" **/
                    acc_num.real = acc_num.real - *(num_coefficient[i]) * pow(mif_private->circuit.frequency,i) * (*gain); 
                }
            }
        ...
        for (i=0; i<den_size; i++) {
            ...
            if (modf(divide_integer/2.0,&temp) > 0.0 ) { /* Negative coefficient       */
                                                     /* values for this iteration. */
                if (frac > 0.0 ) {  /** Odd Powers of "s" **/
                    acc_den.imag = acc_den.imag - *(den_coefficient[i]) * pow(mif_private->circuit.frequency,i); 
                }
                else {                      /** Even Powers of "s" **/
                    acc_den.real = acc_den.real - *(den_coefficient[i]) * pow(mif_private->circuit.frequency,i); 
                }
            }
            else {               /* Positive coefficient values for this iteration */
                if (frac > 0.0 ) {  /** Odd Powers of "s" **/
