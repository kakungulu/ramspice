...
void cm_slew(Mif_Private_t *mif_private)   

{
   ...
   if (mif_private->circuit.init == 1) { 
    ...
    /* DC analysis, get old values */ 
            ...
            *ins = *in_old = mif_private->conn[0]->port[0]->input.rvalue;   
            *outs = *out_old = *ins;
        ...
        mif_private->conn[1]->port[0]->partial[0].port[0] = 1; 

    }else

    if (mif_private->circuit.anal_type == MIF_TRAN) {     /**** DC & Transient Analyses ****/

        /** Retrieve frequently used parameters... **/

        slope_rise = mif_private->param[0]->element[0].rvalue;
        slope_fall = mif_private->param[1]->element[0].rvalue;
        ...
        if ( mif_private->circuit.time == 0.0 ) {         /*** Test to see if this is the first ***/
     ...
     /* determine the slope of the input */
     	...
     	*ins = mif_private->conn[0]->port[0]->input.rvalue;
        ...
        mif_private->conn[1]->port[0]->output.rvalue = *outs;          
