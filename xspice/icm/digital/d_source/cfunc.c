...
#line 1 "./digital/d_source/cfunc.mod"
...
FILE *fopen_with_path_cfunc2(const char *path, const char *mode)
{
    char buf[1024+1];
...
void cm_d_source(Mif_Private_t *mif_private)
{
...
/*********************************************************************/
    ...
    if(mif_private->circuit.init) {  /* initial pass */
        ...
        char* filename = mif_private->param[0]->element[0].svalue;
        source = fopen_with_path_cfunc2( filename, "r");

        if (!source) {
            char *lbuffer, *p;
            lbuffer = getenv("NGSPICE_INPUT_DIR");
            if (lbuffer && *lbuffer) {
                p = (char*) malloc(strlen(lbuffer) + strlen(DIR_PATHSEP) + strlen(mif_private->param[0]->element[0].svalue) + 1);
                sprintf(p, "%s%s%s", lbuffer, DIR_PATHSEP, mif_private->param[0]->element[0].svalue);
                source = fopen(p, "r");
                free(p);
            }
            if (!source) {
                char msg[512];
                snprintf(msg, sizeof(msg), "cannot open file %s", mif_private->param[0]->element[0].svalue);
                cm_message_send(msg);
            }
        }
        ...
        cm_event_alloc(1, mif_private->conn[0]->size * (int) sizeof(char));
        ...
        *row_index = 0;
        ...
        loc->width = mif_private->conn[0]->size;
        ...
        *row_index = *row_index_old;
        ...
        for (i=0; i<mif_private->conn[0]->size; i++) bits[i] = bits_old[i];
    ...
    if ( 0.0 == mif_private->circuit.time ) {
        ...
        if ( 0.0 == test_double && loc->depth > 0 ) { /* Set DC value */
            ...
            for (i=0; i<loc->width; i++) {
                ...
                ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->state = out.state;
                ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->strength = out.strength;
            }
            ...
            for(i=0; i<loc->width; i++) {
              ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->state = UNKNOWN;
              ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->strength = UNDETERMINED;
            }
        }
    }
    else {
        ...
        if ( mif_private->circuit.time < test_double ) { /* Breakpoint has not occurred */

            /** Output hasn't changed...do nothing this time. **/
            for (i=0; i<loc->width; i++) {
                mif_private->conn[0]->port[i]->changed = FALSE;
            }
        ...
        if ( mif_private->circuit.time == test_double ) { /* Breakpoint reached */
            ...
            for (i=0; i<loc->width; i++) {
                ...
                ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->state = out.state;
                mif_private->conn[0]->port[i]->delay = 1.0e-10;
                ((Digital_t*)(mif_private->conn[0]->port[i]->output.pvalue))->strength = out.strength;
            }
