...
foreach vgs (5 5.5 6 6.5)
    foreach vds (5 6 6.5 7 7.5)
	    ...
	    echo "Characterizing $rez"
            utils/reduced_characterize.tcl -vgs_rez $vgs -vds_rez $vds -vbs_rez $vbs -l_rez $l -tech $tech -topv $topv -device nch 
            utils/reduced_characterize.tcl -vgs_rez $vgs -vds_rez $vds -vbs_rez $vbs -l_rez $l -tech $tech -topv $topv -device pch 
	    foreach dev (nch pch)
	       foreach corner (ss tt ff fs sf) 
	          foreach interpolation (LUT) 
                     ...
                     # utils/reduced_analyze_stats.tcl -topv $topv -tech $tech -device $dev -process $corner -size 500 -source $RAMSPICE/Tech_DB/$tech/reduced/$rez -interpolation $interpolation > $RAMSPICE/Tech_DB/$tech/reduced/$rez/$dev.$corner.$interpolation.stats
		  end
	       end	
	    end	
        end
