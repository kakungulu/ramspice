...
load_tech
set ::rez [join [list $vgs_rez $vbs_rez $l_rez] :]
...
if {![file exists $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode]} {
    file mkdir $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode
}
if {![file exists $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}]} {
    file mkdir $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}
}
...
foreach type [split $device :] {
    ...
    set vt_db_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_vt.db
    set va_db_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_va.db
    set min_vt_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_min_vt.tcl
    set ids_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_ids.db
    set gm_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_gm.db
    set ro_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_ro.db
    set vth_mis_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_vth_mis.db
    set ids_mis_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_ids_mis.db
    if {[file exists $vt_db_file]&&[file exists $min_vt_file]&&[file exists $va_db_file]} {
        ...
        foreach ::corner $::corner_list {
		    ...
		    set W [expr $L*$wmin/$lmin]
                    ...
                    mosfet mn_${i3} $type G G ${i3} B $L $W
                ...
                foreach L $l_values {
                    ...
                    ^ @ look_up_tables/$type/diode/Vt/${::corner}($i3) = $Vt
		    if {$i3>3} {
                ...
                ::spice::dc vgs [expr $max_supply-$epsilon] $max_supply $epsilon 
                set i2 1
                set i3 0
                foreach L $l_values {
                    set Ids_low  [get_spice_data V(${i2}) 0]
                    ...
                    ^ @ look_up_tables/$type/diode/Va/${::corner}($i3) = $Va
                    incr i3
                    incr i2
                }  
            }
        ...
        foreach ::corner $::corner_list {
            foreach array $short_views {
                @ /look_up_tables/$type/diode/$array/${::corner}([llength $l_values]) !
            }
            foreach array $short_views {
                set i3 0
                foreach L $l_values {
                    array_set_legend /look_up_tables/$type/diode/$array/${::corner} 0 $i3 $L
                    incr i3
                }
            }
        }
	...
	Info: Saving Arrays
        @ /look_up_tables/$type/diode/Vt save $vt_db_file
        @ /look_up_tables/$type/diode/Va save $va_db_file
        set minVt 0.6
        Info: minVt=$minVt
	...
	set ohmic_factor [expr 1e12*$lmin/$wmin]
        foreach ::corner $::corner_list {
		    ...
		    set W [expr $L*$wmin/$lmin]
                ...
                Info:  Measuring Ids(Vgs,Vbs) gm(Vgs,Vbs) and ro(Vgs,Vbs)
                ...
                ::spice::dc vgs $::constraints(Vgs,minval) $::constraints(Vgs,maxval) $::constraints(Vgs,step) vbs $::constraints(Vbs,minval) $::constraints(Vbs,maxval) $::constraints(Vbs,step)
                Info: done Ids running. Saving results. ([clock format [clock seconds]])
                save_characterization_slice /tmp/Ids.$::corner $index_range 2 1 0 $ohmic_factor
                ::spice::dc vgs [expr $::constraints(Vgs,minval)+$::epsilon] [expr $::constraints(Vgs,maxval)+$::epsilon] $::constraints(Vgs,step) vbs $::constraints(Vbs,minval) $::constraints(Vbs,maxval) $::constraints(Vbs,step) 
                Info: done gm running. Saving results. ([clock format [clock seconds]])
                save_characterization_slice_differential /tmp/gm.$::corner $index_range 2 1 0 [expr $ohmic_factor/$::epsilon]
                ::spice::dc vgs $::constraints(Vgs,minval) $::constraints(Vgs,maxval) $::constraints(Vgs,step) vbs $::constraints(Vbs,minval) $::constraints(Vbs,maxval) $::constraints(Vbs,step) 
                Info: done ro running. Saving results. ([clock format [clock seconds]])
                save_characterization_slice_differential /tmp/ro.$::corner $index_range 2 1 0 /[expr $::epsilon/$ohmic_factor] 
        ...
        foreach ::corner $::corner_list {
            ...
            lappend index_range [llength $l_values]
            foreach array $views {
                @ look_up_tables/$type/diode/$array/${::corner}([join $index_range ,]) !
            }
            foreach array $views {
                foreach_in_range Vgs i0 { 
                    array_set_legend /look_up_tables/$type/diode/$array/$::corner 0 $i0 $Vgs
                }
                foreach_in_range Vbs i2 {
                    array_set_legend /look_up_tables/$type/diode/$array/$::corner 1 $i2 $Vbs
                }
                set i3 0
                foreach L $l_values {
                    array_set_legend /look_up_tables/$type/diode/$array/$::corner 2 $i3 $L
                    incr i3
                }
            }
        }
        ...
        foreach ::corner $::corner_list {
            load_characterization_slice /look_up_tables/$type/diode/Ids/$::corner /tmp/Ids.$::corner
            load_characterization_slice /look_up_tables/$type/diode/gm/$::corner /tmp/gm.$::corner
            load_characterization_slice /look_up_tables/$type/diode/ro/$::corner /tmp/ro.$::corner
        }
        Info: Saving Array
        @ /look_up_tables/$type/diode/Ids save $ids_file
        @ /look_up_tables/$type/diode/gm save $gm_file
        @ /look_up_tables/$type/diode/ro save $ro_file
        # Cleanup
        foreach char_file [glob -nocomplain /tmp/char_vig_task*] {
            file delete $char_file
        }
	Info: Ids=[expr [@ /look_up_tables/$type/diode/Ids/ss calc 1.8 0 180e-9]*220/180]
    }
    exit
    if {![file exists $vth_mis_file]||![file exists $ids_mis_file]} {
        ...
        foreach ::corner $::corner_list {
            ...
            fork_task char_mis_task {
 		    ...
 		    set W [expr $L*$wmin/$lmin]
                   # mosfet {name type D G S B L W Lmin}
                    mosfet mn_${i3} $type G G ${i3} 0 $L $W
                    netlist "r_${i3} ${i3} 0 1e-12"
                ...
                foreach L $l_values {
                    ...
                    ^ @ /look_up_tables/$type/diode/Vth_mis/${::corner}($i3) = [expr $Sigma/$Vth]
                    incr i
                    set Ids [lindex $result $i]
                    incr i
                    set Sigma [lindex $result $i]
                    ^ @ /look_up_tables/$type/diode/Ids_mis/${::corner}($i3) = [expr $Sigma/$Ids]
                    incr i
                    incr i3
                }    
            }
        ...
        foreach ::corner $::corner_list {
            ...
            ######### Initialize database
            foreach array $views {
                Info: New Array: look_up_tables/$type/diode/$array/${::corner}([llength $l_values])
                @ look_up_tables/$type/diode/$array/${::corner}([llength $l_values]) !
            }
            foreach array $views {
                set i3 0
        ...
        foreach ::corner $::corner_list {
            set ::temp $::corner_to_temp($::corner)
            foreach array $views {
                generate_lut  /look_up_tables/$type/diode/$array/$::corner
            }
        }
        @ /look_up_tables/$type/diode/Vth_mis save $vth_mis_file
        @ /look_up_tables/$type/diode/Ids_mis save $ids_mis_file
        # Cleanup
    ...
    foreach ::corner $::corner_list {
        set thermal_noise_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_${::corner}_thermal_noise.db
        set flicker_noise_file $::env(RAMSPICE)/Tech_DB/${tech}/4d_diode/${::rez}/${tech}_${type}_diode_${::corner}_flicker_noise.db
    ...
    if {!$noise_complete} {
        ...
        proc noise_cont {coord} {
            @ look_up_tables/$type/diode/flicker_const/${::corner}($coord) = 0
        ...
        foreach ::corner $::corner_list {
            ...
            for {set section 1} {[info exists bin(n,$section,lmin)]} {incr section} {
                ...
                # mosfet {name type D G S B L W Lmin}
                mosfet mn_0_0 $type G G 3 B $bin(n,$section,lmin) $bin(n,$section,wmin) $section
                netlist {
                    .end
                }
                update_netlist
                    ...
                    foreach_in_range Vgs i0 {
                        ::spice::alter vgs = $Vgs
                            foreach_in_range Vbs i2 {
                                ...
                                if {$gm==0} {
                                    @ look_up_tables/$type/diode/flicker_const/${::corner}($i0,$i2,$i3) =  [expr $dummy_flicker/$dummy_count_flicker]
                                    @ look_up_tables/$type/diode/thermal_noise/${::corner}($i0,$i2,$i3) = [expr $dummy_thermal/$dummy_count_thermal]
                                } else {
                                    if {[catch {@ look_up_tables/$type/diode/flicker_const/${::corner}($i0,$i2,$i3) =  [expr sqrt($flicker_noise*$W*$L/($gm*$gm))]}]} {
                                        @ look_up_tables/$type/diode/flicker_const/${::corner}($i0,$i2,$i3) = [expr $dummy_flicker/$dummy_count_flicker]
                                    } else {
                                        set dummy_flicker [expr $dummy_flicker+sqrt($flicker_noise*$W*$L/($gm*$gm))]
                                        incr dummy_count_flicker
                                    }
                                    if {[catch {@ look_up_tables/$type/diode/thermal_noise/${::corner}($i0,$i2,$i3) = [expr sqrt($thermal_noise/$gm)]}]} {
                                        @ look_up_tables/$type/diode/thermal_noise/${::corner}($i0,$i2,$i3) = [expr $dummy_thermal/$dummy_count_thermal]
                                    } else {
                                ...
                                #Info: thermal_noise=[@ look_up_tables/$type/diode/thermal_noise/${::corner}($i0,$i2,$i3)]
                                #Info: flicker_noise=[@ look_up_tables/$type/diode/flicker_const/${::corner}($i0,$i2,$i3)]
                                ::spice::destroy all
                                ::spice::alter vgs = $Vgs
                            }
                            ETA
            ...
            @ /look_up_tables/$type/diode/thermal_noise/$::corner save ${thermal_noise_file}
            @ /look_up_tables/$type/diode/flicker_const/$::corner save ${flicker_noise_file}
        }
    }
}
exit
