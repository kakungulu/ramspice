...
static int db_tuner___tuner_analyze_btree __P((DB_ENV *, DB *, u_int32_t));
static int db_tuner___tuner_ff_stat_callback __P((DBC *, PAGE *, void *, int *));
static int db_tuner___tuner_generate_fillfactor_stats __P((DB_ENV *, DB *,
    TUNER_FF_STAT *));
static int db_tuner___tuner_insert_dupdata __P((DB *, u_int32_t, int, TUNER_FF_STAT *));
static int db_tuner___tuner_insert_kvpair __P((DB *, u_int32_t, u_int32_t, int, int,
    int, TUNER_FF_STAT *));
static int db_tuner___tuner_leaf_page __P((DBC *, PAGE *, TUNER_FF_STAT *));
static int db_tuner___tuner_leaf_dupdata __P((DBC*, PAGE *, int, int, u_int32_t,
    TUNER_FF_STAT *));
static int db_tuner___tuner_leaf_dupdata_entries __P((DBC *, PAGE *, int, int, int, int,
    TUNER_FF_STAT *));
static int db_tuner___tuner_opd_data_entries __P((DBC *, PAGE *, int, int,
    TUNER_FF_STAT *));
static int db_tuner___tuner_opd_data __P((DBC *, PAGE *, int, int, TUNER_FF_STAT *));
static int db_tuner___tuner_opd_page __P((DBC *, PAGE *, TUNER_FF_STAT *));
static int db_tuner___tuner_print_btree_fillfactor __P((u_int32_t, TUNER_FF_STAT *));
static int db_tuner___tuner_record_dup_pg __P((int, TUNER_FF_STAT *));
static int db_tuner___tuner_record_last_opd __P((int, TUNER_FF_STAT *));
static int db_tuner___tuner_record_leaf_pg __P((int, TUNER_FF_STAT *));
	...
	char *args;
{
	...
	__db_util_arg("db_tuner", args, &argc, &argv);
	return (db_tuner_main(argc, argv) ? EXIT_FAILURE : EXIT_SUCCESS);
}
	...
	char *argv[];
{
	...
	switch (dbtype) {
	case DB_BTREE:
		if ((ret = db_tuner___tuner_analyze_btree(dbenv, dbp, verbose)) != 0)
 ...
 */
static int
db_tuner___tuner_analyze_btree(dbenv, dbp, verbose)
	DB_ENV *dbenv;
	DB *dbp;
	u_int32_t verbose;
{
	...
	if ((ret = db_tuner___tuner_generate_fillfactor_stats(dbenv, dbp,
	    &stats)) != 0) {
		dbenv->err(dbenv, ret,
	...
	(void)db_tuner___tuner_print_btree_fillfactor(verbose, &stats);
...
static int
db_tuner___tuner_generate_fillfactor_stats(dbenv, dbp, stats)
	DB_ENV *dbenv;
	DB *dbp;
	TUNER_FF_STAT *stats;
{
	...
	if ((ret = __bam_traverse(dbc, DB_LOCK_READ, PGNO_INVALID, 
	    db_tuner___tuner_ff_stat_callback, (void *)stats)) != 0) {
	...
	for (i = 0; i < NUM_PGSIZES; ++i) {
		if (stats->pg_leaf_offset[i] > 0 &&
		    (ret = db_tuner___tuner_record_leaf_pg(i, stats)) != 0) {
			dbenv->err(dbenv, ret, "__tuner_record_leaf");
			break;
		}

		if (stats->pg_dup_offset[i] > 0 &&
		    (ret = db_tuner___tuner_record_dup_pg(i, stats)) != 0) {
 ...
 */
static int 
db_tuner___tuner_ff_stat_callback(dbc, h, cookie, putp)
	DBC *dbc;
	PAGE *h;
	void *cookie;
	int *putp;
{
	...
	*putp = 0;
	...
	switch (TYPE(h)) {
	case P_LBTREE:
		if ((ret = db_tuner___tuner_leaf_page(dbc, h, cookie)) != 0) {
	...
	case P_LRECNO:
		...
		if (h->prev_pgno == PGNO_INVALID &&
		    (ret = db_tuner___tuner_opd_page(dbc, h, cookie)) != 0) {
 ...
 */
static int
db_tuner___tuner_leaf_page(dbc, h, cookie)
	DBC *dbc;
	PAGE *h;
	TUNER_FF_STAT *cookie;
{
	...
	for (indx = 0; indx < top;) {
		...
		if ((indx + P_INDX) < top && inp[indx] == inp[indx + P_INDX]) {
			...
			/*  Count the key once. */
			onpd_sz += db_tuner_item_space(dbp, h, indx);
			...
			for (findx = indx;
				...
				onpd_sz += db_tuner_item_space(dbp, h, indx + O_INDX);
			}
			...
			if ((ret = db_tuner___tuner_leaf_dupdata(dbc, h, findx, lindx,
			    onpd_sz, cookie)) != 0) {
			...
			/* First consider key. */
			key_sz = db_tuner_item_size(dbp, h, indx);
			...
			if (B_TYPE(GET_BKEYDATA(dbp, h,
				...
				data_sz = db_tuner_item_size(dbp, h, indx + O_INDX);

			for (i = 0; i < NUM_PGSIZES; ++i) {
				if ((ret = db_tuner___tuner_insert_kvpair(dbp, key_sz,
 ...
 */
static int
db_tuner___tuner_leaf_dupdata(dbc, h, findx, lindx, dup_sz, cookie)
	DBC *dbc;
	PAGE *h;
	int findx, lindx;
	u_int32_t dup_sz;
	TUNER_FF_STAT *cookie;
{
	...
	for (i = 0; i < NUM_PGSIZES; ++i) {
		...
		if ((ret = db_tuner___tuner_leaf_dupdata_entries(dbc, h, findx,
		 ...
		 */
		if (is_opd &&
		    (ret = db_tuner___tuner_record_last_opd(i, cookie)) != 0) {
 ...
 */
static int
db_tuner___tuner_leaf_dupdata_entries(dbc, h, findx, lindx, indx_pgsz, is_opd, cookie)
	DBC *dbc;
	PAGE *h;
	...
	TUNER_FF_STAT *cookie;
{
	...
	for (indx = findx; indx < lindx; indx += P_INDX) {
		...
		if (indx == findx) {
			 ...
			 */
			if (is_opd && (ret =
			    db_tuner___tuner_insert_kvpair(dbp, key_sz, 0, indx_pgsz,
		...
		if (is_opd) {
			ret = db_tuner___tuner_insert_dupdata(dbp, data_sz,
			    indx_pgsz, cookie);
			if (ret != 0) {
				dbenv->err(dbenv, ret,
			...
			ret = db_tuner___tuner_insert_kvpair(dbp, key_sz, data_sz,
...
static int
db_tuner___tuner_opd_page(dbc, h, cookie)
	DBC *dbc;
	PAGE *h;
	TUNER_FF_STAT *cookie;
{
	...
	if ((ret = db_tuner_get_opd_size(dbc, h, &opd_sz)) != 0) {
	...
	for (i = 0; i < NUM_PGSIZES; ++i) {
		...
		if ((ret = db_tuner___tuner_opd_data(dbc, h, i, is_opd, cookie)) != 0) {
...
static int
db_tuner___tuner_opd_data(dbc, h, indx_pgsz, is_opd, cookie)
	DBC *dbc;
	PAGE *h;
	...
	TUNER_FF_STAT *cookie;
{
	 ...
	 */
	if (is_opd) {
		ret = db_tuner___tuner_insert_kvpair(dbp, 0, 0, indx_pgsz,
	...
	while (1) {
		ret = db_tuner___tuner_opd_data_entries(dbc, h, indx_pgsz, is_opd,
		    cookie);
		if (ret != 0) {
	...
	if (is_opd && (ret = db_tuner___tuner_record_last_opd(indx_pgsz, cookie)) != 0) {
		dbenv->err(dbenv, ret, "__tuner_record_last_opd");
		return (ret);
	}
 ...
 */
static int
db_tuner___tuner_opd_data_entries(dbc, h, indx_pgsz, is_opd, cookie)
	DBC *dbc;
	PAGE *h;
	int indx_pgsz, is_opd;
	TUNER_FF_STAT *cookie;
{
	...
	for (indx = 0; indx < NUM_ENT(h); indx += O_INDX) {
		...
		data_sz = db_tuner_item_size(dbp, h, indx);
		...
		if (is_opd) {
			 ...
			 */
			ret = db_tuner___tuner_insert_kvpair(dbp, 0, data_sz,
 ...
 */
static int
db_tuner___tuner_insert_kvpair(dbp, key_sz, data_sz, indx_pgsz, in_key, in_data, stats)
	DB *dbp;
	...
	TUNER_FF_STAT *stats;
{
	...
	if (is_big_key) {
		...
		if ((ret = 
		    db_tuner___tuner_record_ovfl_pg(key_sz, indx_pgsz, stats)) != 0) {
	...
	if (is_big_data) {
		...
		if ((ret =
		    db_tuner___tuner_record_ovfl_pg(data_sz, indx_pgsz, stats)) != 0) {
		 ...
		 * db_tuner___tuner_leaf_page, in which case, only the key is inserted
	...
	if ((stats->pg_leaf_offset[indx_pgsz] + needed) > pgsize) {
		...
		if ((ret = db_tuner___tuner_record_leaf_pg(indx_pgsz, stats)) != 0) {
...
static int
db_tuner___tuner_insert_dupdata(dbp, data_sz, indx_pgsz, stats)
	DB *dbp;
	u_int32_t data_sz;
	int indx_pgsz;
	TUNER_FF_STAT *stats;
{
	...
	if ((stats->pg_dup_offset[indx_pgsz] + needed) > pgsize) {
		/* no enough space then record current page info. */
		if ((ret = db_tuner___tuner_record_dup_pg(indx_pgsz, stats)) != 0) {
			dbenv->err(dbenv, ret, "__tuner_record_dup_pg");
			return (ret);
		}
...
static int
db_tuner___tuner_record_ovfl_pg(size, indx_pgsz, stats)
	u_int32_t size;
	int indx_pgsz;
	TUNER_FF_STAT *stats;
{
...
static int
db_tuner___tuner_record_leaf_pg(indx_pgsz, stats)
	int indx_pgsz;
	TUNER_FF_STAT *stats;
{
	int dist;
...
static int
db_tuner___tuner_record_dup_pg(indx_pgsz, stats)
	int indx_pgsz;
	TUNER_FF_STAT *stats;
{
	int dist;
 ...
 */
static int
db_tuner___tuner_record_last_opd(indx_pgsz, stats)
	int indx_pgsz;
	TUNER_FF_STAT *stats;
{
	int ret;
	if (stats->pg_dup_offset[indx_pgsz] != 0 && 
	    (ret = db_tuner___tuner_record_dup_pg(indx_pgsz, stats)) != 0)
		return (ret);
 ...
 */
static int
db_tuner_get_opd_size(dbc, h, opd_sz)
	DBC *dbc;
	PAGE *h;
	u_int32_t *opd_sz;
{
	...
	while (1) {
		dup_sz += db_tuner_sum_opd_page_data_entries(dbp, p);
...
static int
db_tuner_sum_opd_page_data_entries(dbp, h)
	DB *dbp;
	PAGE *h;
{
	db_indx_t i;
	...
	for (i = 0; i < NUM_ENT(h); i += O_INDX) {
		...
		if (B_DISSET(GET_BKEYDATA(dbp, h, i)->type))
			continue;
		sz += db_tuner_item_space(dbp, h, i);
	}
...
static int
db_tuner___tuner_print_btree_fillfactor(verbose, stats)
	u_int32_t verbose;
	TUNER_FF_STAT *stats; 
{
...
static int
db_tuner_open_db(dbpp, dbenv, dbname, subdb)
	DB **dbpp;
	DB_ENV *dbenv;
	char *dbname;
	char *subdb;
