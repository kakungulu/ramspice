...
::SVG::svg width $outer_frame_size height $outer_frame_size {
    ::SVG::graph_markers x 100 y 0 width $frame_size height $frame_size data  {1e6 1 2e6 0.5 3e6 0.33 4e6 0.25 5e6 0.2 6e6 0.167} markers 4:green connect all x_title $x y_title $y x_unit $x_unit y_unit $y_unit
}
</td>
<td bgcolor=$::colors(red)><b>Property</b></td><td bgcolor=$::colors(red)><b>Required</b></td><td bgcolor=$::colors(red)><b>Selected</b></td></tr>
set i 0
foreach key [lsort -unique [concat [array names ::SESSION] $selected_g]] {
    ...
    if {[lsearch $selected_g $key]==-1} {
        <tr bgcolor=$::colors($color)><td><b>$::properties($key,html)</b></td><td>[eng $::opt($key) $::properties($key,unit)]</td><td></td></tr>
    } else {
        <tr bgcolor=$::colors($color)><td><b>$key</b></td><td>
...
<tr bgcolor=$::colors(green)><td colspan="3" align="center">
...
foreach sizer $::sizer_list {
    set color [lindex {yellow orange} [expr $i%2]] 
    <tr bgcolor=$::colors($color)><td><b>$sizer</b></td><td colspan="3"><input type=range id="$sizer" min="$::sizers($sizer,min)"  max="$::sizers($sizer,max)" style="width:${::svg_width}px"></td></tr>
    incr i 
}
</table>
return
