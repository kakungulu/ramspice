  ...
  <script> 
	  ...
	  append ::spec_js_code "<table ><tr><td colspan=\"3\"><td></td><td></td><td></td><td></td><b>Specifications:</b></td><td rowspan=\"[expr [llength $::topologies($topology,properties)]+1]\">"
	  set ::web_output ::spec_js_code
	  draw_schematic $topology
	  set ::web_output ::analyze_js_code
	  draw_schematic $topology
	  set ::web_output 1
	  append ::spec_js_code "</td><td id=\"${topology}_analysis\" rowspan=\"[expr [llength $::topologies($topology,properties)]+1]\">No Analysis Yet</td></tr>"
	  foreach property $::property_list {
	      ...
	      if {$property=="Name"} {
	          append ::spec_js_code "<tr><td></td><td></td><td></td><td></td><td><b>$::topologies($topology,$property,html)</b></td><td><input size=\"8\" type=\"string\" id=\"$property\"  $value onChange=\"SendSpecToServer()\;\"></td><td><b>$::topologies($topology,$property,unit)</b></td></tr>"
	      } else {
		  ...
		  append ::spec_js_code "<td><b>$::topologies($topology,$property,html)</b></td><td><input size=\"8\" type=\"number\" id=\"$property\"  $value onChange=\"SendSpecToServer()\;\"></td><td><b>$::topologies($topology,$property,unit)</b></td></tr>"
	      }
	      append ::ajax_send_code "+ \"&$property=\" + escape(document.getElementById(\"$property\").value)"
	  }
	  append ::spec_js_code "<tr><td></td><td></td><td></td><td></td><td colspan=\"3\" align=\"center\" id=\"AnalysisWindox\"></td></tr>"
	  ...
	  for (i=0;i<selected.length;i++) {
          ...
          var win=document.getElementById("AnalysisWindox");
	...
	win.innerHTML="";
     ...
     function toggle_g(id) {
	  ...
	  for (i=0;i<selected_g.length;i++) {
	      if (selected_g[i]==id) {
	          document.getElementById(selected_g[i]).innerHTML='$::goal_icon_gray';
		  selected_g[i]=selected_g[selected_g.length-1];
		  selected_g.pop();
		  return
	      }
	  }
	  document.getElementById(id).innerHTML='$::goal_icon_black';
	  selected_g.push(id);
 	  update_analysis();
      }
