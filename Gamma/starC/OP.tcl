 ...
 #       *c "composite_gamma_gcc_interpolate_4(&@look_up_tables:${type}:Ids:ss:LUT,&@look_up_tables:${type}:gm:ss:LUT,&@look_up_tables:${type}:ro:ss:LUT,&(@$transistor:gm),&(@$transistor:go),&(@$transistor:Ids),(@$g:V)-(@$s:V),(@$d:V)-(@$s:V),(@$b:V)-(@$s:V),@$L,@$W);"
	...
	*c "@$::opt(topology):leak=0;"
	...
	Info: $expression($node)
        $node=>($expression($node))*@$::opt(topology):Ted
        skip {$node=="vdd"}
    ...
    *c "if (op_it>=@$::opt(topology):config:op_iterations) \{@$::opt(topology):status:fail=@$::opt(topology):leak; return TCL_ERROR;\}"
    ...
    ############
    #####################################################################################################
    *c "@$::opt(topology):Det=$expression(Det);"
    if {$::debug_mode} {*c "printf(\"Det=%g\\n\",@$::opt(topology):Det);"}
    *c "@$::opt(topology):Ted=1/@$::opt(topology):Det;"
    ...
    *c "@$::opt(topology):property:Rout=fabs(($expression(Rout))*@$::opt(topology):Ted);"
    #####################################################################################################
    ############
    ############   AC Properties
    ############
	...
	*c "//AC gain: $spectrum_num/$expression(Det_ac)"
    ...
    Info: AC
    set expression(dDet_ac) [derive_expression @$::opt(topology):s $expression(Det_ac)]
...
#    *c "      printf(\"F=%g fraction=(%g,%g/%g,%g) Adc=%g A=%g/%g=%g phase=%g\\n\",W/(2*3.141592656),snr,sni,sdr,sdi,@$::opt(topology):property:Adc*@$::opt(topology):property:Adc,BW_Mag0,BW_Mag,BW_Mag/BW_Mag0,atan(sni/snr)-atan(sdi/sdr));"
    ...
    *c "for (BW_it=0;BW_it<5;BW_it++)  @$::opt(topology):s-=($expression(Det_ac))/($expression(dDet_ac));"
    *c "@$::opt(topology):property:BW:s=$expression(dDet_ac);"
    *c "@$::opt(topology):p1=-@$::opt(topology):s;"
    ...
    *c "@$::opt(topology):s-=1e3;"
    # Find next root (=pole)
    *c "for (BW_it=0;BW_it<20;BW_it++)  @$::opt(topology):s-=(($expression(Det_ac))*(@$::opt(topology):s+@$::opt(topology):p1-5e2))/(($expression(dDet_ac))*(@$::opt(topology):s+@$::opt(topology):p1-5e2)-($expression(Det_ac)));"
    *c "@$::opt(topology):p2=-@$::opt(topology):s;"
	...
	*c "if (!isfinite(@$transistor:Nf))  \{@$::opt(topology):status:fail=6; return TCL_ERROR;\}"
    ...
    foreach transistor $::all_transistors {
        *c "float current_transfer_$transistor=@$::opt(topology):Ted*([DERIVE @$transistor:Ideq $out_node_expression])/@$::opt(topology):property:Adc;"
