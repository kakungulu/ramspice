    ...
    *c "float step;"
    *c "while (1) \{"
    *c "    if (p->content->num_of>@config:pat_size_target) break;"
    ...
    *c "    printf(\"               %ld/%g=%g%%\\n\",p->content->num_of,@config:pat_size_target,100*p->content->num_of/@config:pat_size_target);"
    *c "    fflush(stdout);"
    *c "    if (@param:unique>0) \{"
    *c "        pat_unique(p,@param:unique);"
    *c "    printf(\"               post unique: %ld/%g=%g%%\\n\",p->content->num_of,@config:pat_size_target,100*p->content->num_of/@config:pat_size_target);"
    *c "    if (p->content->num_of>0.92*@config:pat_size_target) break;"
    *c "\}"
    *c "\}"
    *c "    if (p->content->num_of>@config:pat_size_target) break;"
    *c "\}"
