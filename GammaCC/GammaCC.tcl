...
proc .size {name = value {min {}} {max {}} {step {}} {dependence {}}} {
    ...
    Info: New SIZE $name=[@ /size/$name] params=[@ param list]
...
proc .dependence {args} {
 ...
 #   regsub -all {@+} $dep "@$::opt(topology):" dep
...
namespace eval C {
    ...
    variable code_template {
        ...
        static int tcl_gamma_$!_export_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            LOCAL_BUFFER_RETURN_GOES_HERE
            return TCL_OK;
        }
        static int tcl_gamma_$!_op_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
        ...
        //      static int tcl_gamma_grad_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            ...
            //         return TCL_OK;
        //    }
        static int tcl_gamma_$!_heatmap_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            HEATMAP_CODE_GOES_HERE
            return TCL_OK;
        }
        static int tcl_gamma_$!_random_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            RANDOM_CODE_GOES_HERE
            return TCL_OK;
        }
        static int tcl_gamma_$!_breed_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            BREED_CODE_GOES_HERE
            return TCL_OK;
        }
        static int tcl_gamma_$!_random_breed_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            RANDOM_BREED_CODE_GOES_HERE
            return TCL_OK;
        }
        static int tcl_gamma_$!_random_breed_single_cmd(ClientData clientData,Tcl_Interp *interp,int argc,char *argv[]) {
            RANDOM_BREED_SINGLE_CODE_GOES_HERE
            return TCL_OK;
        }
        // Initializing cTree references and registering the tcl_gamma_op_cmd command as ::C::@name
        int Gamma_$!_Init(Tcl_Interp *interp) {
            float *array_entry;
            context *c;
            GLOBAL_POINTER_INIT_GO_HERE
            return TCL_OK;
        }
        int Gamma_$!_Register(Tcl_Interp *interp) {
            Tcl_CreateCommand(interp, "::Gamma::$!::heatmap", tcl_gamma_$!_heatmap_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::random", tcl_gamma_$!_random_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::random_breed", tcl_gamma_$!_random_breed_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::random_breed_single", tcl_gamma_$!_random_breed_single_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::breed", tcl_gamma_$!_breed_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::op", tcl_gamma_$!_op_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::import", tcl_gamma_$!_import_cmd, NULL, NULL);
            Tcl_CreateCommand(interp, "::Gamma::$!::export", tcl_gamma_$!_export_cmd, NULL, NULL);
            return TCL_OK;
        }
    }
}
...
proc gcc {name {preprocess 1}} {
    if {$preprocess} {
	    ...
	    set total_line_num [llength $code]
            foreach line $code {
	        incr line_num
	        Info: line $line_num/$total_line_num [regexp {^\s*\#[A-Z][a-z]+:\s} $line] [string range $line 0 40]
		if {![regexp @ $line]} {
		    append linked_code $line
		    append linked_code \n
		    continue
 		}
		if {[regexp {^\s*\#[A-Z][a-z]+:\s} $line]} {
		    Info: as is
		    append linked_code $line
		    append linked_code \n
		    continue
 		}
		regsub -all {&[^@]} $line { } context_strings
		regsub -all {(&?)@+([A-Za-z0-9_:]+)[^&@]*} $context_strings {\1@\2 } context_strings
		regsub {^[^@]*(&@)} $context_strings {\1} context_strings
		if {![regexp {^&@} $context_strings]} {
		    regsub {^[^@]*(@)} $context_strings {\1} context_strings
		}
		set context_strings [lsort -unique -decreasing $context_strings]
                foreach context_string $context_strings {
		    regsub {^.*@} $context_string {} context_string
                    if {[info exists pointer_names($context_string)]} {
                        regsub -all "&@+$context_string" $line $pointer_names($context_string) line
                        regsub -all "@+$context_string" $line $var_names($context_string) line
                        continue
                    }
                    if {[regexp {^[0-9]} $context_string]} {
                        regsub -all {[^a-zA-Z_0-9]} CONST_$context_string _ var_name
                    ...
                    lappend used_pointer_names $pointer_name
                    regsub -all "&@+$context_string" $line $pointer_name line
                    regsub -all "@+$context_string" $line $var_name line
                    if {![info exists var_names($context_string)]} {
                        Info: Linking $context_string to cTree
                        append global_pointers "float *$pointer_name;\n"
                        append global_variables "float $var_name;\n"
                        if {[regexp {(.*):PAT} $context_string -> base]} {
