...
source $::env(RAMSPICE)/GammaCC/GammaCC.tcl
get_opts
...
source $::env(RAMSPICE)/GammaCC/simplify.tcl
source $::env(RAMSPICE)/GammaCC/matrices.tcl
source $::env(RAMSPICE)/GammaCC/derivatives.tcl
source $::env(RAMSPICE)/GammaCC/polynomials.tcl
...
proc add_idc {name m p value} {
    Info: 1 add_idc params=[@ param list]
    set ::idc($m,$p) $value
    Info: 2 add_idc params=[@ param list]
    add_node $m $p
    Info: 3 add_idc params=[@ param list]
}
...
proc add_node {args} {
    Info: 1 add_node $args params=[@ param list]
    foreach m $args {
        Info: 2 add_node params=[@ param list]
        ladd ::all_nodes $m
        Info: 3 add_node params=[@ param list]
        if {![@ $m:V ?]} {
        Info: 4 add_node $m params=[@ param list]
            @ $m:V = 0
        Info: 5 add_node $m params=[@ param list]
        }
        Info: 6 add_node params=[@ param list]
    }
        Info: 7 add_node params=[@ param list]
}
...
.compile_circuit
if {[file exists $::env(RAMSPICE)/Etc/Templates/$::opt(topology)/$::opt(tech).db]} {
    Warning: PAT already exists in $::env(RAMSPICE)/Etc/Templates/$::opt(topology)/$::opt(tech).db
    Info: Done
    exit
}
...
@ /$::opt(topology)/circuits(([join $pareto_sizes ,]|[join $pareto_properties ,])) !
Info: Creating skeleton PAT in $::env(RAMSPICE)/Etc/Templates/$::opt(topology)/$::opt(tech).db
@ / save $::env(RAMSPICE)/Etc/Templates/$::opt(topology)/$::opt(tech).db
Info: Done
exit
