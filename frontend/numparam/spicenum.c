...
nupa_init(char *srcfile)
{
    ...
    incontrolS = 0;
    placeholder = 0;
    dicoS = (tdico *) new_string(sizeof(tdico));
    initdico(dicoS);
...
static void
return_symbol(tdico *dico_p, NGHASHPTR htable_p, char *name_in, char *val_out)
{
    char *name;                 /* current symbol */
    entry *entry_p;             /* current entry */
    NGHASHITER iter;            /* hash iterator - thread safe */
...
static void
set_symbol(tdico *dico_p, NGHASHPTR htable_p, char *name_in, char *val_in)
{
    char *name;                 /* current symbol */
    entry *entry_p;             /* current entry */
    NGHASHITER iter;            /* hash iterator - thread safe */
...
void
nupa_get_params(char *name,char *val)
{
    int depth;                  /* nested subcircit depth */
    tdico *dico_p;              /* local copy for speed */
    NGHASHPTR htable_p;         /* current hash table */
    ...
    dico_p = dicoS;
    if (dico_p == NULL) {
        fprintf(cp_err, "\nWarning: No symbol table available for 'get param'\n");
        return;
    }
    for (depth = dico_p->stack_depth; depth > 0; depth--) {
        htable_p = dico_p->local_symbols[depth];
        if (htable_p) {
            return_symbol(dico_p, htable_p, name,val);
	    return;
        }
    }
    return_symbol(dico_p, dico_p->global_symbols, name,val);
}
void
nupa_set_params(char *name,char *val)
{
    int depth;                  /* nested subcircit depth */
    tdico *dico_p;              /* local copy for speed */
    NGHASHPTR htable_p;         /* current hash table */
    ...
    for (depth = dico_p->stack_depth; depth > 0; depth--) {
        htable_p = dico_p->local_symbols[depth];
        if (htable_p) {
            set_symbol(dico_p, htable_p, name,val);
	    return;
        }
    }
    set_symbol(dico_p, dico_p->global_symbols, name,val);
}
