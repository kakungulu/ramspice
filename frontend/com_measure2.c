...
{
    if (!chk_only) {
        #Error: "measure  %s  %s(%s) : %s" mName mFunction trigTarg errMsg
    }
}
    ...
    )
{
    ...
    if (d == NULL) {
        #Error: "no such vector as %s." meas->m_vec
        return;
    }

    if (has_d2 && (d2 == NULL)) {
        #Error: "no such vector as %s." meas->m_vec2
        return;
    }
    if (dScale == NULL) {
        #Error: "no scale vector." 
        return;
    }
    ...
    )
{
    ...
    if (dScale == NULL) {
        #Error: "no such vector time, frequency or dc." 
        return;
    }
    ...
    )
{
    ...
    if (ac_check || sp_check) {
        ...
        #Error: "no such analysis type as %s." meas->m_analysis
        return;
    }
    ...
    *result = 0.0e0;        /* default result */
    ...
    while (words) {
        ...
        case 2:
        {
            ...
            if (mFunctionType == AT_UNKNOWN) {
                if (!autocheck) {
                    #Error: "\tmeasure '%s'  failed" mName
                    #Error: "measure  %s  :" mName
                    #Error: "\tno such function as '%s'" words->wl_word
                }
                return MEASUREMENT_FAILURE;
            }
            break;
    ...
    case AT_TRIG:
    {
        ...
        if (out_line)
            ...
            #Info: "%-20s=  %e targ=  %e trig=  %e"  mName  (measTarg->m_measured-measTrig->m_measured)  measTarg->m_measured  measTrig->m_measured
...
err_ret1:
    ...
    case AT_FIND:
    {
        ...
        if (out_line)
            ...
            #Info: "%-20s=  %e" mName meas->m_measured
...
err_ret3:
    ...
    case AT_INTEG:
    {
        ...
        if (out_line)
            ...
            #Info: "%-20s=  %.*e from=  %.*e to=  %.*e"  mName  precision  meas->m_measured  precision  meas->m_from  precision  meas->m_to
...
err_ret4:
    ...
    case AT_AVG:
    {
        ...
        if (out_line)
            ...
            #Info: "%-20s=  %e from=  %e to=  %e"  mName  meas->m_measured  meas->m_at  meas->m_measured_at
    ...
    case AT_MAX_AT:
    {
        ...
        if ((mFunctionType == AT_MIN) || (mFunctionType == AT_MAX)) {
            ...
            if (out_line)
                ...
                #Info: "%-20s=  %e at=  %e"  mName  measTrig->m_measured  measTrig->m_measured_at
            ...
            *result = measTrig->m_measured;
            ...
            if (out_line)
                ...
                #Info: "%-20s=  %e with=  %e"  mName  measTrig->m_measured_at  measTrig->m_measured
...
err_ret6:
    ...
    case AT_PP:
    {
        ...
        if (out_line)
            ...
            #Info: "%-20s=  %e from=  %e to=  %e"  mName  (maxValue-minValue)  measTrig->m_from  measTrig->m_to
...
err_ret7:
    ...
    case AT_ERR3:
    {
        #Error: "\tmeasure '%s'  failed" mName
        #Error: "measure  %s  :" mName
        #Error: "\tfunction '%s' currently not supported" mFunction
        break;
    }

    default:
    {
        #Error: "enumeration value `AT_UNKNOWN' not handled in get_measure2 Aborting..."
