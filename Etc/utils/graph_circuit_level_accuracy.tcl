...
source $::env(RAMSPICE)/Etc/Templates/$::opt(topology)/regression.tcl
array set data {
    bw {}
    pm {}
}
...
for {set i 0} {[info exists sample($i)]} {incr i} {
    ...
    skip {abs(165-[lindex $sample($i) 6]-[lindex $sample($i) 7])>10}
    lappend data(pm) [expr 165-[lindex $sample($i) 6]]
    lappend data(pm) [lindex $sample($i) 7]
}
...
foreach view [array names data] {
    ...
    ::SVG::svg width 1000 height 1100 {
        ::SVG::graph_data font 32 x 100 y 100 width 900 height 900 data $data($view) show_unit 0 markers 6:green x_title SPICE y_title Gamma x_unit $unit($view) y_unit $unit($view) title "$view values"
    }
    </td>
