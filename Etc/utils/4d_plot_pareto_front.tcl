...
set html_file $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d/$::opt(device).$::opt(process).$::opt(interpolation).$::opt(view).$opt(type).pareto.html
if {[file exists $html_file]} {
#    Info: HTML report exists: $html_file
#    exit
}    
set pareto_data_file $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d/$::opt(device).$::opt(process).$::opt(interpolation).pareto.tcl
...
#        Info: $int <> $sim ===> [expr (1.0*$int/$sim-1)*100]
    ...
    foreach source [lsort [glob $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d/*:*:*:*]] {
