...
set html_file $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d_diode/$::opt(device).$::opt(process).$::opt(interpolation).$::opt(view).$opt(type).pareto.html
if {[file exists $html_file]} {
#    Info: HTML report exists: $html_file
#    exit
}    
set pareto_data_file $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d_diode/$::opt(device).$::opt(process).$::opt(interpolation).pareto.tcl
proc calc_error {parameter section sim int} {
    ...
    #    if {$sim<0} return
    #    if {$int<0} return
    #    Info: Including $::Vgs $::Vbs | $int-$sim in $parameter $section [expr ($int/$sim-1)*100]
    if {$sim==0} {
        Warning: sim=0
	set sim 1e-99
    }
...
#        Info: $int <> $sim ===> [expr (1.0*$int/$sim-1)*100]
    ...
    foreach source [lsort [glob $::env(RAMSPICE)/Tech_DB/$::opt(tech)/4d_diode/*:*:*]] {
        set rez [file tail $source]
        set input_file $source/$::opt(device).diode.$::opt(process).$::opt(interpolation).info
        if {![file exists $input_file]} {
	    Warning: Missing Input File: $input_file
	    continue
	}    
	Info: Reading $input_file
        ...
        source $source/$::opt(tech)_$::opt(device)_diode_min_vt.tcl
        set p [string index $::opt(device) 0]
        open_bin read $input_file
	set sample_size [read_bin int]
	    ...
	    set access_time($rez) -1
            for {set j 0} {$j<$sample_size} {incr j} {
                foreach var {Vgs Vbs L W LUT_Ids Ids LUT_gm gm LUT_ro ro} {
...
if {$opt(type)=="volume"} {
    puts $O "<table border=\"2\"><tr><td></td><td><b>Vgs</b></td><td><b>Vbs</b></td><td><b>L</b></td><td><b>Memory</b></td><td><b>Max Error</b></td></tr>"
} else {
    puts $O "<table border=\"2\"><tr><td></td><td><b>Vgs</b></td><td><b>Vbs</b></td><td><b>L</b></td><td><b>Time</b></td><td><b>Max Error</b></td></tr>"
}
set index 1
foreach rez [lsort -command volume_sort [array names volume]] {
    skip {![info exists ninety_nine_rel($rez,$::opt(view))]}
