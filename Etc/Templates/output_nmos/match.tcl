...
#	Info: $s=$spice_op($s)
    ...
    @ / foreach_child node {
        if {[info exists spice_op($node)]} {
	        ...
	        Info: Overflow: $node=$spice_op($node) iref=[eng [@ size:iref] A] W=[eng [@ size:W] m] L=[eng [@ size:L] m]
	        set skip_this 1
	        break
	    }
	}
 ...
 #   Info: inoise_spectrum=[get_spice_data inoise_spectrum all]
    ...
    set prev_Aac $Adc
    set Aac3dB 0
    for {set i 0} {$i<[get_spice_data $out_node length]} {incr i} {
...
#    @ property:BW = [expr ((pow(10,[@ property:Adc]/20)-1))*1.1*[@ property:BW]]
    @ property:BW = [expr ((pow(10,[@ property:Adc]/20)-1))*[@ property:BW]/20]
    set BW_error [expr abs(100*($BW/[@ property:BW]-1))]
    if {$BW_error>12} {
        Info: skipped [expr $BW/[@ property:BW]]
	continue
    }
...
#		puts -nonewline $O "$c = [eng [expr 100*([@ /$t/$c]/[set Captured_C[regsub {^c} $c {}]]-1)] %]\t"
    ...
    Info: Weighted error=[eng [expr 100*($gamma_acc/$spice_acc-1)] %]
...
#    skip {abs($gamma_pm-$spice_pm)>20}
    ...
    set entry {}
#    set O [open $regression_sample_file a]
    puts $O [list set sample($S) [concat $L $Lp $Aac0 [@ property:Adc] $CMRR [@ property:CMRR] $PSRR [@ property:PSRR] $BW [@ property:BW] $spice_pm $gamma_pm] <br>]
     close $O
     incr S
