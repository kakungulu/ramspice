...
TSKtask *task, CKTnode *gnode, JOB *foo)
{
    ...
    #For: {set i 1} {$i<=$::ANALYSIS_NESTING_DEPTH} {incr i} {
        if (*line) {
            ...
            INPinsert(&name, tab);
            ptemp.uValue = name;
            GCA(INPapName, (ckt, which, foo, "name$i", &ptemp));
            parm = INPgetValue(ckt, &line, IF_REAL, tab); /* vstart$i */
            GCA(INPapName, (ckt, which, foo, "start$i", parm));
            parm = INPgetValue(ckt, &line, IF_REAL, tab); /* vstop$i */
            GCA(INPapName, (ckt, which, foo, "stop$i", parm));
            parm = INPgetValue(ckt, &line, IF_REAL, tab); /* vinc$i */
            GCA(INPapName, (ckt, which, foo, "step$i", parm));
        }
    }
    return 0;
}
...
INP2dot(CKTcircuit *ckt, INPtables *tab, card *current, TSKtask *task, CKTnode *gnode)
{
    ...
    strcmp(token, ".print") == 0 || strcmp(token, ".plot") == 0) {
        /* obsolete - ignore */
        #Warning: "obsolete control card - ignored"
        goto quit;
